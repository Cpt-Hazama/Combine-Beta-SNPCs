...
util.AddNPCClassAlly(CLASS_COMBINE,"npc_fassassin")
...
function ENT:OnInit()
	...
	self:slvSetHealth(GetConVarNumber("sk_fassassin_health"))
...
function ENT:EventHandle(...)
	...
	if(event == "rattack") then
		...
		if(!self:SLV_IsPossesed() && IsValid(self.entEnemy) && self.entEnemy:Health() > 0) then
...
function ENT:OnDanger(vecPos,iType)
	...
	self:SLVPlayActivity(tbFlipAct[flip])
	return true
end
...
function ENT:_PossJump(entPossessor,fcDone)
	local dir = entPossessor:KeyDown(IN_MOVELEFT) && FLIP_RIGHT ||
				...
				FLIP_FORWARD
	self:SLVPlayActivity(tbFlipAct[dir],false,fcDone)
end
...
function ENT:_PossSecondaryAttack(entPossessor, fcDone)
	self:SLVPlayActivity(math.random(1,2) == 1 && ACT_MELEE_ATTACK1 || ACT_MELEE_ATTACK2,false,fcDone)
end
	...
	if(disp == D_HT) then
		...
		if(self:CanSee(enemy)) then
			if(dist <= self.fMeleeDistance || distPred <= self.fMeleeDistance) then
				...
				else self:SLVPlayActivity(ACT_MELEE_ATTACK1,true) end
				return
			end
